...
def cron_clients_nodejs_update():
    ...
    with TemporaryDirectory() as tmp_dir:
      ...
      diff_ns = subprocess.check_output(
          shlex.split('git diff --name-status'), cwd=client_lib_dir)
...
def cron_clients_ruby_update():
    ...
    with TemporaryDirectory() as tmp_dir:
      ...
      _call(('git clone'
             ' https://github.com/saicheems/google-api-ruby-client'
             ' {}').format(client_lib_dir), check=True)
      ...
      cmd = cmd.format(account.name, account.email, date.today().isoformat())
      # A zero return code means there's something to push.
      if _call(cmd, cwd=client_lib_dir) == 0:
        remote_url = ('https://{}:{}@github.com'

...
_DEVNULL = open(os.devnull, 'w')  # /dev/null
...
"""
"""string: The pom.xml used to build Java samples."""

Context = collections.namedtuple('Context', ('discovery_document_filename'
                                             ' override_filenames id_ name'
                                             ' version revision'))
"""Contains the execution context of a single API.

   Args:
       discovery_document_filename (string): A Discovery document filename.
       override_filenames (list): A list of override filenames.
       id_ (string): The API ID.
...
"""Contains the information required to run a sample.

   Args:
       id_ (string): The method ID.
...
def _generate_overrides(test_dir, discovery_document_filename, name, version):
    ...
    For the given Discovery document, this method generates two override files:
    1. A mock value override which overrides strings with defined patterns.
    2. A mock auth/discoveryDocUrl override which forces the sample to be
    ...
    # 2. Generate the auth/discoveryDocUrl override.
    misc_override_filename = os.path.join(src_dir, 'override2.json')
...
def _generate_samples(ctx, language, ruby_names_file=None):
    ...
    # Map of language names to their respective file extensions.
    ext = {
        _CSHARP: 'cs', _GO: 'go', _JAVA: 'java', _NODEJS: 'njs',
        _PHP: 'php', _PYTHON: 'py', 'ruby': 'rb'
    }[language]
    # Glob a list of filenames of the generated samples.
...
def _load_csharp(test_dir, ctxs):
    ...
    for ctx in ctxs:
        ...
        for filename in sample_filenames:
            ...
            # .../csharp/foo.bar.get/foo.bar.get.csproj
...
def _load_java(test_dir, ctxs):
    ...
    for ctx in ctxs:
        ...
        for i, filename in enumerate(sample_filenames):
            ...
            # ./src/main/java/foo.bar.get/FooSample.java
            new_filename = os.path.join(package_dir,
                                        '{}.java'.format(sample_class_name))

            # Prepend "package p{i};" to each sample.
...
def _load_php(test_dir, ctxs):
    ...
    for ctx in ctxs:
        ...
        # Write the composer.json file, which points to
        # lib/google-api-php-client-services.
